                ...
                while(byIndex < (byLen - 1))
                {
                    switch(byMDBCLBuffer[byIndex])
                    {
                        ...
                        case RSP_BUSY: //12 lecteur occupé
                            // <editor-fold defaultstate="collapsed" desc="RSP_BUSY">
                        {
                            byIndex++;
                            break;
                        }// </editor-fold>
...
static void vTaskBeatingHearth(void *pvParameter)
{
    ...
    {
        isAllPolled = FALSE;
        if(isMDBPolled && isKBDpOLLED && isUSBPolled && isDisplayPolled &&
           isFlipDisplayPolled)
        {
            isAllPolled = TRUE;
            isMDBPolled = isKBDpOLLED = isUSBPolled = isDisplayPolled =
                isFlipDisplayPolled = FALSE;
            ClrWdt();
        }
        if((UsageMode == MODEUSER) && isAllPolled)
        {
            LEDTOGGLE(RED);
        }
