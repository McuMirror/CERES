...
DWORD dwGetMaxReturn(void)
{
    ...
    if(isBillValidatorReseted)
    {
        ...
        {
            if(((centralParameters.billType.byBillEnable[1] >> byIndex) & 0x01) && billValidatorConfig.byBillValue[byIndex])
            {
                result = billValidatorConfig.byBillValue[byIndex] * billValidatorConfig.wScaleFactor;
            }
