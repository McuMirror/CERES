...
int main(void)
{
    ...
    if(
       ...
       xTaskCreate(vTaskTest, "TEST", STACKTEST, NULL, 2, hTest) &&
...
//CashLess
                    ...
                    else
                    {
                        switch(byMDBCGBuffer[byIndex])
                        {
                            ...
                            case JUST_RESET_CG:
                            {
                                ...
                                if(isMDBSetupConfig(CGADDRESS, &changeGiverConfig, sizeof (ChangeGiverConfig)))
                                {
        ...
        else
        {
                ...
                for(byIndex = 0; byIndex < byLen - 1; byIndex++)
                {
                    ...
                    else
                    {
                        switch(byMDBBVBuffer[byIndex])
                        {
                            case JUST_RESET_BV:
                            {
                                if((isBillValidatorReseted = isMDBSetupConfig(BVADDRESS, &billValidatorConfig,
                                                                              sizeof (BillValidatorConfig))))
                                {
                                    ...
                                    isBillValidatorReseted = isGetMDBStackerStatus(byStacker);
                                }
                                break;
                            }
                            default:
            ...
            isBillValidatorReseted = ((byLen > 0) && (byLen < 32));
        }
        //End billValidator
...
void vTaskDoChange(void *pvParameter)
{
    ...
    //        //        lInitialAmount = lAmount = lAmountInCashDispo / (isChangerReseted ? changeGiverConfig.deviceConfig.byScaleFactor : 1);
    //        lAmount = 0;
    //        if(isChangeAutorized && lAmountInCashDispo && isGetMDBTubeStatus(&tubeStatus)) //Vérifie si il y a un montant à distribuer.
    //        {
    //            isDispensed = TRUE;
    //            isChangeAutorized = FALSE;
    ...
    //                            }
    //                            if(byToBeDispensed && (isDispenseMDBChanger(byIndex, byToBeDispensed)))
    //                            {
...
void vTaskTest(void *pvParameter)
{
    for(;;)
    {
        ...
        LCD_CURSOR_OFF();
        LCDClear();
        sprintf(byBuffer, "%s: %s %s", byTexte[TEST_START_OP], byTexte[EXIT], byTexte[CONTINUE]);
...
BYTE byCheckMachine(const BYTE byNumber)
{
    ...
    if(!byResult)
    {
        ...
        sprintf(byBuffer, "%s %u: ", byTexte[NUMMACHINE], byNumber); //, isKeyPresent ? byTexte[AMOUNT] : "");
...
void vEnableCashDevice(const BOOL isEnable, const BYTE byIndexMachine)
{
    ...
    if(isBillValidatorReseted)
    {
        ...
        isEnableBillValidator(isEnable && ((byIndexMachine == 100) || modules[byIndexMachine].config.isBillAccepted),
                              centralParameters.isEscrow && billValidatorConfig.byEscrow,
...
BOOL isCodeCheck(WORD wEEAddress)
{
...
void vStartBuzzer(BOOL isActive, int iBuzzerLen)
{
    if(isActive)
    {
...
void vRecordMachineData()
{
                             ...
                             modules[byIndexSelectedMachine].config.byNumero))
    {
        ...
        vSetMainPowerRelayState(modules[byIndexSelectedMachine].config.byNumero, !(ESTATE)modules[byIndexSelectedMachine].config.isMachinePowered);
        isSetOverBusyDelay(modules[byIndexSelectedMachine].config.byNumero, modules[byIndexSelectedMachine].config.lOverBusy);
    }
    else
    {
