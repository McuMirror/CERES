...
BOOL boSendByteToE2promL(BYTE byData)
{
    BYTE byLoop = 0XFF;
...
BOOL SetE2promAddress(WORD wAddress)
{
    ...
    {
        if(!boSendByteToE2promL(byBuffer[byIndex]))
        {
            return FALSE;
        }
    }
...
BOOL I2CE2promLWriteBuffer(const WORD wAddress, void *data, const size_t size)
{
    ...
    if(SetE2promAddress(wAddress))
    {
        ...
        {
            if(!boSendByteToE2promL(byData[byIndex]))
            {
                return FALSE;
            }
        }
...
BOOL I2CE2promLReadBuffer(const WORD wAddress, void *data, const size_t size)
{
    ...
    else
    {
        ...
        byLoop = 0xFF;
        if(boSendByteToE2promL(EE_ADDR | EEREAD))
        {
            while(!I2CTransmissionHasCompleted(I2CPort))
            {
                if(!byLoop--)
...
BOOL I2CE2promLReadByte(const WORD wAddress, BYTE *byData)
{
