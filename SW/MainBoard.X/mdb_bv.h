...
BOOL boBillValidatorReseted;

BOOL boSetMDBillType(BillType *billType);

BOOL boGetMDBStackerStatus(BYTE *wStacker);

/**
 * \fn boEnableBillValidator
 * \brief Active ou desactive le lecteur de  billets.
 * \param[in] boEnable Indique si le lecteur de  billets doit être activé ou non.
 * \param[in] billType Paramètrage d'acceptation des billets de la centrale.
 * \param[in] boEscrow Indique si l'escrow doit être activé.
