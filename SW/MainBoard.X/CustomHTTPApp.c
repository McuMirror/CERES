...
void UpdateCashActivation(BOOL boCG);
...
BYTE HTTPCheckAuth(BYTE* cUser, BYTE* cPass)
{
    ...
    {
        sprintf(byBuffer, "%u", centralParameters.passWords.httpTechnician);
        if(strcmppgm2ram((char *)cPass, byBuffer) == 0)
        {
    ...
    {
        sprintf(byBuffer, "%u", centralParameters.passWords.httpAccountant);
        if((strcmppgm2ram((char *)cPass, byBuffer) == 0) ||
    ...
    //    if(!isHttpPasswordValid)
    //    {
    //        vSetHisto((byResult < 0x80) ? atoi(cPass) : 0, 0, PASSWORDENTRANCE, 0, (byResult >= 0x080));
    //    }
...
HTTP_IO_RESULT HTTPExecuteGet(void)
{
    ...
    {
        modules[byIndexMachineHTPP].config.boCumulEnable = (HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"EnableCumul") > NULL);
        modules[byIndexMachineHTPP].config.boDisplayedTimeRemaining = (HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"EnableTimeDisplay") > NULL);
        modules[byIndexMachineHTPP].config.boMachinePowered = (HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"PowerSupply") > NULL);
        modules[byIndexMachineHTPP].config.boBillAccepted = (HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"BillAccepted") > NULL);
        ...
        vSetMainPowerRelayState(modules[byIndexMachineHTPP].config.byNumero,
                                !(ESTATE)modules[byIndexMachineHTPP].config.boMachinePowered);
        if(ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"BasePrice"))
        {
            modules[byIndexMachineHTPP].config.wCashPrice = (int)(strtof((char*)ptr, NULL) * 100);
        }
        if(ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"KeyPrice"))
        {
            modules[byIndexMachineHTPP].config.wCashLessPrice = (int)(strtof((char*)ptr, NULL) * 100);
        }
        if(ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"HHPrice"))
        {
            modules[byIndexMachineHTPP].config.wCashPriceHH = (int)(strtof((char*)ptr, NULL) * 100);
        }
        if(ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"HHKeyPrice"))
        {
            modules[byIndexMachineHTPP].config.wCashLessPriceHH = (int)(strtof((char*)ptr, NULL) * 100);
        }
        ...
        boSetSensibility(modules[byIndexMachineHTPP].config.byNumero,
                         modules[byIndexMachineHTPP].config.BusyLevel.wBusyLevel);
        vSetMainPowerRelayState(modules[byIndexMachineHTPP].config.byNumero, !(ESTATE)modules[byIndexMachineHTPP].config.boMachinePowered);
        boSetOverBusyDelay(modules[byIndexMachineHTPP].config.byNumero, modules[byIndexMachineHTPP].config.lOverBusy);
    ...
    {
        centralParameters.boBuzzer = (HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"buzzer") > NULL);
        centralParameters.boEscrow = (HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"escrow") > NULL);
        if(ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"Appoint"))
        {
            centralParameters.wAmountExactChange = (int)(atof((char*)ptr)) * 100;
        }
    ...
    if(!memcmppgm2ram(filename, "tubes.htm", strlen(filename)))
    {
        ...
        {
            lAmountInCashDispo = (int)(strtof((char*)ptr, NULL) * 100);
...
{
    BYTE bylBuffer[16];
    sprintf(bylBuffer, "%02X%02X%02X%02X%02X%02X",
...
void HTTPPrint_Val(WORD byIndex)
{
    ...
    if(boFormCG)
    {
        sprintf(bylBuffer, "%.*f", byDecimalPos, changeGiverConfig.byCoinValue[byIndex] == 0XFF ? (double)(0) :
                (double)(changeGiverConfig.byCoinValue[byIndex] * changeGiverConfig.deviceConfig.byScaleFactor) /
                wCurrencyDivider);
    }
    else
    {
        sprintf(bylBuffer, "%.*f", byDecimalPos, billValidatorConfig.byBillValue[byIndex] == 0XFF ? (double)(0) :
...
{
    BYTE bylBuffer[8];
    sprintf(bylBuffer, "%u", tubeStatus.byLevel[byIndex]);
    TCPPutString(sktHTTP, bylBuffer);
}
...
void HTTPPrint_Op(void)
{
    ...
    switch(lHistoData.Operation)
    {
        ...
        {
            sprintf(bylBuffer, "%s", "Saisie password");
            break;
        }
        case INITIALISATION:
        {
            sprintf(bylBuffer, "%s", "Mise en route");
            break;
        }
        case TEST_START:
        {
            sprintf(bylBuffer, "%s", "Entree test");
            break;
        }
        case LANCEMENT:
        {
            sprintf(bylBuffer, "%s", "Activation");
            break;
        }
        case INCG:
        {
            sprintf(bylBuffer, "%s", "Insert. pieces");
            break;
        }
        case OUCG:
        {
            sprintf(bylBuffer, "%s", "Rendu");
            break;
        }
        case INBV:
        {
            sprintf(bylBuffer, "%s", "Insert. billets");
            break;
        }
        case OUTBV:
        {
            sprintf(bylBuffer, "%s", "Retour billets");
            break;
        }
        case PAYCASHLESS:
        {
            sprintf(bylBuffer, "%s", "Pai. cashless");
            break;
        }
        case REVALUECASHLESS:
        {
            sprintf(bylBuffer, "%s", "Rech. cashless");
            break;
        }
        case KEYPRESENT:
        {
            sprintf(bylBuffer, "%s", "Clef presentee");
            break;
        }
        case OVERPAYOP:
        {
            sprintf(bylBuffer, "%s", "Trop percu");
            break;
        }
        case FILLINGOP:
        {
            sprintf(bylBuffer, "%s", "Remplissage");
            break;
        }
        case USBKEYINSERTED:
        {
            sprintf(bylBuffer, "%s", "USB Presentee");
            break;
        }
        case USBPAY:
        {
            sprintf(bylBuffer, "%s", "Paiement USB");
            break;
        }
        case USBSAVEAUDIT:
        {
            sprintf(bylBuffer, "%s", "Save audit USB");
            break;
        }
        case USBREINITCODE:
        {
            sprintf(bylBuffer, "%s", "USB init code");
            break;
        }
        case RAZAUDITOP:
        {
            sprintf(bylBuffer, "%s", "RAZ audit");
            break;
        }
        case USBSAVEHISTO:
        {
            sprintf(bylBuffer, "%s", "Save histo USB");
            break;
        }
        case SELECTION:
        {
            sprintf(bylBuffer, "%s", "Selection");
            break;
        }
        case REFUSED:
        {
            sprintf(bylBuffer, "%s", "Piece refusee");
            break;
        }
        case TEST_LEFT:
        {
            sprintf(bylBuffer, "%s", "Sortie test");
            break;
        }
        case USB_LEFT:
        {
            sprintf(bylBuffer, "%s", "Retrait USB");
            break;
        }
        case PULSE_END:
        {
            //Version 1.2.1
            //2015 10 10
            sprintf(bylBuffer, "%s", "Fin paiement ou activation");
            //sprintf(bylBuffer, "%s", "Fin activation machine");
            //-------------
            break;
        }
        default:
        {
            sprintf(bylBuffer, "%s", "Inconnu");
        }
    }
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[12];
    sprintf(bylBuffer, "%02X-%02X-%02X", lHistoData.date.mday, lHistoData.date.mon, lHistoData.date.year);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[3];
    lHistoData.byMachine ? sprintf(bylBuffer, "%u", lHistoData.byMachine) : sprintf(bylBuffer, "-");
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[9];
    sprintf(bylBuffer, "%02X:%02X:%02X", lHistoData.time.hour, lHistoData.time.min, lHistoData.time.sec);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[8];
    lHistoData.lAmount || (lHistoData.Operation == OUCG) ? sprintf(bylBuffer, "%s%.*f",
                                                                   (lHistoData.Operation == OUCG) ? "-" : "", byDecimalPos,
                                                                   (double)(lHistoData.lAmount * changeGiverConfig.deviceConfig.byScaleFactor) / wCurrencyDivider) :
        sprintf(bylBuffer, "-");
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[9];
    lHistoData.lKeyNumber ? sprintf(bylBuffer, "%u", lHistoData.lKeyNumber) : sprintf(bylBuffer, "-");
    TCPPutString(sktHTTP, bylBuffer);
}
...
void HTTPPrint_total(void)
{
    ...
    sprintf(bylBuffer, "%.*f", byDecimalPos, (double)(dwTotalTubes * changeGiverConfig.deviceConfig.byScaleFactor) / wCurrencyDivider);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[4];
    sprintf(bylBuffer, "%s", lHistoData.result ? "OK" : "NOK");
    TCPPutString(sktHTTP, bylBuffer);
}

void HTTPPrint_buzzer(void)
{
    centralParameters.boBuzzer ? TCPPutROMString(sktHTTP, (ROM BYTE*)"checked") :
        TCPPutROMString(sktHTTP, (ROM BYTE*)"");
}

void HTTPPrint_escrow(void)
{
    centralParameters.boEscrow ? TCPPutROMString(sktHTTP, (ROM BYTE*)"checked") :
        TCPPutROMString(sktHTTP, (ROM BYTE*)"");
}
...
{
    BYTE bylBuffer[8] = {0};
    sprintf(bylBuffer, "%.*f", byDecimalPos, (double)centralParameters.wAmountExactChange / wCurrencyDivider);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[8];
    sprintf(bylBuffer, "%u", centralParameters.byCumulTO);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[8];
    sprintf(bylBuffer, "%u", centralParameters.byKBDTO);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[8];
    sprintf(bylBuffer, "%u", centralParameters.byOverPayTO);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[8];
    sprintf(bylBuffer, "%u", centralParameters.lDelayDisplay);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[6];
    sprintf(bylBuffer, "%02XH%02X", Now.hour, Now.min);
    TCPPutString(sktHTTP, bylBuffer);
}
...
void HTTPPrint_machineList(void)
{
    ...
    for(byNumMachine = 10; byNumMachine < 100; byNumMachine++)
    {
        ...
        {
            sprintf(byShortBuffer, "%u-", byNumMachine);
            strcat(bylBuffer, byShortBuffer);
        }
    }
    bylBuffer[strlen(bylBuffer) - 1] = 0;
    TCPPutString(sktHTTP, bylBuffer);
...
{
    BYTE bylBuffer[8] = {0};
    sprintf(bylBuffer, "%.*f", byDecimalPos,
...
{
    BYTE bylBuffer[10];
    sprintf(bylBuffer, "%u", modules[byIndexMachineHTPP].config.lPulseInMS);
    TCPPutString(sktHTTP, bylBuffer);
}

void HTTPPrint_boRemaining(void)
{
    modules[byIndexMachineHTPP].config.boDisplayedTimeRemaining ? TCPPutROMString(sktHTTP, (ROM BYTE*)"checked") :
        TCPPutROMString(sktHTTP, (ROM BYTE*)"");
}
...
{
    BYTE bylBuffer[10];
    sprintf(bylBuffer, "%u", modules[byIndexMachineHTPP].config.lTimeToDisplay);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[10];
    sprintf(bylBuffer, "%u", modules[byIndexMachineHTPP].config.lOverBusy);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[12];
    sprintf(bylBuffer, "%02X:%02X", modules[byIndexMachineHTPP].config.utBeginSelect.hour,
...
{
    BYTE bylBuffer[12];
    sprintf(bylBuffer, "%02X:%02X", modules[byIndexMachineHTPP].config.utEndSelect.hour,
...
{
    BYTE bylBuffer[12];
    sprintf(bylBuffer, "%02X:%02X", modules[byIndexMachineHTPP].config.utBeginHH.hour,
...
{
    BYTE bylBuffer[12];
    sprintf(bylBuffer, "%02X:%02X", modules[byIndexMachineHTPP].config.utEndHH.hour,
            modules[byIndexMachineHTPP].config.utEndHH.min);
    TCPPutString(sktHTTP, bylBuffer);
}

void HTTPPrint_boCumul(void)
{
    modules[byIndexMachineHTPP].config.boCumulEnable ? TCPPutROMString(sktHTTP, (ROM BYTE*)"checked") :
        TCPPutROMString(sktHTTP, (ROM BYTE*)"");
}
...
{
    BYTE bylBuffer[2];
    sprintf(bylBuffer, "%u", byIndHisto);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[5];
    sprintf(bylBuffer, "%u", auditData.data.wNumberChannelCGIn[wIndex]);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[5];
    sprintf(bylBuffer, "%u", auditData.data.wNumberChannelCGOut[wIndex]);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[5];
    sprintf(bylBuffer, "%u", auditData.data.wNumberChannelBV[wIndex]);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[5];
    sprintf(bylBuffer, "%u", ((byIndHisto = byMachineExist(byNummch)) < MAXMACHINES) ? auditData.data.wNumberServices[byIndHisto] : 0);
    TCPPutString(sktHTTP, bylBuffer);
}

void HTTPPrint_boPower(void)
{
    modules[byIndexMachineHTPP].config.boMachinePowered ? TCPPutROMString(sktHTTP, (ROM BYTE*)"checked") :
        TCPPutROMString(sktHTTP, (ROM BYTE*)"");
}

void HTTPPrint_boBill()
{
    modules[byIndexMachineHTPP].config.boBillAccepted ? TCPPutROMString(sktHTTP, (ROM BYTE*)"checked") :
        TCPPutROMString(sktHTTP, (ROM BYTE*)"");
}
...
{
    BYTE bylBuffer[5];
    sprintf(bylBuffer, "%u", modules[byIndexMachineHTPP].config.BusyLevel.wBusyLevel);
    TCPPutString(sktHTTP, bylBuffer);
}
...
{
    BYTE bylBuffer[10];
    sprintf(bylBuffer, "%s", (billValidatorConfig.byBillValue [(BYTE)wIndex] > 0) ? "" : "none");
    TCPPutString(sktHTTP, bylBuffer);
}
...
void UpdateCashActivation(BOOL boCG)
{
    ...
    for(byIndex = 0; byIndex < MAXNUMBERCHANNELSCG; byIndex++)
    {
        sprintf(bylBuffer, "%u", byIndex);
        if(HTTPGetROMArg(curHTTP.data, bylBuffer) != NULL)
        {
            wCopyEnable += (1 << atoi(bylBuffer));
        }
    }
    boCG ? (centralParameters.coinType.wCoinEnable = WORD_SWAP(wCopyEnable)) :
