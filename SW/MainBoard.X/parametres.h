 ...
 * Author: RA
 *
 * Created on 22 janvier 2014, 04:46
 */
...
/**
 * \typedef CENTRAL_PARAMETERS
 * \brief Structure contenant les paramètres de la centrale.
 */
typedef struct __attribute__((packed))
{
    BYTE boBuzzer; /*!< Booléen indiquant si le buzzer est activé ou non.*/
    BYTE boEscrow; /*!< Booléen indiquant si l'escrow des billets doit être utilisé.*/
...
typedef union
{
    ...
    BYTE byBufferOperation[32];
} __attribute__((packed)) UOPERATION;

/**
 * \union SMACHINE_CONFIG
 * \brief Structure contenant les paramètres à sauvegarder d'une machine.
 */
typedef union
{
    ...
    struct __attribute__((packed))
    {
        ...
        BOOL boCumulEnable; /*!< Autorise ou non le cumul du temps d'activation*/
        BOOL boDisplayedTimeRemaining; /*! Affiche le temps restant.*/
        BOOL boMachinePowered; /*!< Indique si le relais d'alimentation principale doit être alimenté.*/
        BOOL boBillAccepted; /*!<Indique si les billets sont acceptés pour cette machine.*/
        uBusyLevel BusyLevel; /*!< Niveau de detection de l'occupation*/
    };
    BYTE sMachine_config_buffer[64];
} SMACHINE_CONFIG;
