...
#pragma config WDTPS = PS32768          // Watchdog Timer Postscaler (1:1)
...
int main(void)
{
	...
	PORTSetPinsDigitalOut(LED);
	PORTSetPinsDigitalIn(TESTKEY);
	...
	while(--i)
	{
		if(PORTReadBits(TESTKEY) && ValidAppPresent())
		{
			JumpToApp();
		};
	};
 ...
 * Side Effects:    None
 *
 * Overview:        LED flashes alternately fast and slow
 *
 * Note:            None
 ********************************************************************/
void BlinkWait(void)
