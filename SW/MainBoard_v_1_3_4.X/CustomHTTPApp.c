...
HTTP_IO_RESULT HTTPExecuteGet(void)
{
    ...
    if(!memcmppgm2ram(filename, "macParam.htm", strlen(filename)))
    {
        ...
        isSetMainPowerRelayState(modules[byIndexMachineHTPP].config.byNumero,
        ...
        isSetMainPowerRelayState(modules[byIndexMachineHTPP].config.byNumero, !(ESTATE)modules[byIndexMachineHTPP].config.isMachinePowered);
    ...
    if(!memcmppgm2ram(filename, "tubes.htm", strlen(filename)))
    {
        ...
        {
            lAmountInCashDispo = (int)(strtof((char*)ptr, NULL));
...
void HTTPPrint_Val(WORD byIndex)
{
    ...
    {
        snprintf(bylBuffer, sizeof (bylBuffer), "%.*f", byDecimalPos, billValidatorSetup.byBillValue[byIndex] == 0XFF ? (double)(0) :
...
{
    BYTE bylBuffer[7] = {0};
    snprintf(bylBuffer, sizeof (bylBuffer) + 1, "%02XH%02X", Now.hour, Now.min);
    TCPPutString(sktHTTP, bylBuffer);
}
...
void HTTPPrint_machineList(void)
{
    ...
    for(byNumMachine = 10; byNumMachine < 100; byNumMachine++)
    {
        ...
        {
            snprintf(byShortBuffer, sizeof (byShortBuffer) + 1, "%u-", byNumMachine);
...
{
    BYTE bylBuffer[11] = {0};
    snprintf(bylBuffer, sizeof (bylBuffer), "%s", (billValidatorSetup.byBillValue [(BYTE)wIndex] > 0) ? "" : "none");
    TCPPutString(sktHTTP, bylBuffer);
}
